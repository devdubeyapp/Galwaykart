...
public class UpdateAddressActivity extends BaseActivityWithoutCart {
    ...
    String st_attribute_value_mob = "",st_attribute_code="",st_last_name="",st_password="",st_comfirm_pwd="",st_dibid="";
    ...
    public void onBackPressed() {
        ...
        {
           //super.onBackPressed();
            Intent intent = new Intent(UpdateAddressActivity.this, CustomerAddressBook.class);
            intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK | Intent.FLAG_ACTIVITY_NEW_TASK);
            startActivity(intent);
            CommonFun.finishscreen(this);
        ...
        final JsonObjectRequest jsObjRequest = new JsonObjectRequest(Request.Method.GET,getRegAddressURL,null,
                new Response.Listener<JSONObject>() {
                    ...
                    @Override   public void onResponse(JSONObject response) {
                        //Log.d("response_Get",response.toString());
//                        CommonFun.alertError(UpdateAddressActivity.this,response.toString());
                        ...
                        try {
                            ...
                            JSONArray arr_custom_attribute = jsonObj.getJSONArray("custom_attributes");

                            if( arr_custom_attribute.length() > 0){
                                for (int i =0;i<arr_custom_attribute.length();i++) {
                                    JSONObject obj_custom_attribute = arr_custom_attribute.getJSONObject(i);

                                    st_attribute_code = obj_custom_attribute.getString("attribute_code");
                                    if(st_attribute_code.equalsIgnoreCase("mobile_number")) {
                                        st_attribute_value_mob = obj_custom_attribute.getString("value");
                                        pref = CommonFun.getPreferences(getApplicationContext());

                                        SharedPreferences.Editor editor = pref.edit();
                                        editor.putString("st_attribute_value_mob",st_attribute_value_mob);
                                        editor.commit();
...
//            franch_city.setText("");
    ...
    private void updateAddress() {
        ...
        if (st_attribute_value_mob != null && !st_attribute_value_mob.equals("")){
            ...
            st_attribute_value_mob = address_telephone;
        }

        mRequestBody ="{\"customer\":{\"id\":"+login_id+"," +
                "\"group_id\":"+login_group_id+","+
        ...
        //Log.d("mRequestBody_input", mRequestBody);
...
//                            CommonFun.alertError(UpdateAddressActivity.this, response.toString());
                            //Log.d("mRequestBody_output", response);
                            try {
...
//                                CommonFun.alertError(UpdateAddressActivity.this,String.valueOf(response));
                                ...
                                for(int i = 0; i < arr_address_length; i++) {
                                    ...
                                    if (!add_id.equalsIgnoreCase("")) {
                                        ...
                                        editor.putString("log_user_zone",st_company);
                                        editor.commit();
                                        ...
                                        new CountDownTimer(4000, 4000) {
                                            ...
                                            public void onFinish() {
                                                ...
                                                if(is_req_from_checkout==false)
                                                {
                                                    Intent intent=new Intent(UpdateAddressActivity.this, UpdateAddressActivity.class);
                                                    intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TASK|Intent.FLAG_ACTIVITY_NEW_TASK);
                                                    startActivity(intent);
